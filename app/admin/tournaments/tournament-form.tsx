  ...
  return (
    <form action={formAction} className="grid max-w-xl gap-4">
      ...
      <Field label="Sezona" error={state.errors?.seasonId}>
        <select name="seasonId" defaultValue={defaultValues.seasonId} required className="input">
          <option value="">— odaberi —</option>
          {seasons.map((s) => (
            <option key={s.id} value={s.id}>
              {s.system === "GP" ? "Dubrovnik GP" : "GP Akademije"} — {s.yearLabel}
            </option>
          ))}
        </select>
      </Field>

      <Field label="Naziv turnira" error={state.errors?.name}>
        <input name="name" defaultValue={defaultValues.name} required className="input" />
      </Field>

      <div className="grid grid-cols-2 gap-4">
        <Field label="Datum" error={state.errors?.date}>
          <input type="date" name="date" defaultValue={dateValue} required className="input" />
        </Field>
        <Field label="Broj kola" error={state.errors?.rounds}>
          <input
            type="number"
            name="rounds"
      ...
      <div className="grid grid-cols-2 gap-4">
        <Field label="Format" error={state.errors?.format}>
          <select name="format" defaultValue={defaultValues.format ?? "SWISS"} className="input">
            <option value="SWISS">Švicarski</option>
            <option value="ROUND_ROBIN">Kružni (svi protiv svih)</option>
          </select>
        </Field>
        <Field label="Tempo" error={state.errors?.tempo}>
          <select name="tempo" defaultValue={defaultValues.tempo} required className="input">
            <option value="">— odaberi —</option>
      ...
      <Field
        label="Razina (samo za Dubrovnik GP — ostavi prazno za Akademiju)"
        error={state.errors?.level}
      >
        <select name="level" defaultValue={defaultValues.level ?? ""} className="input">
          <option value="">— nije primjenjivo (Akademija) —</option>
          <option value="KLUPSKA">Klupska</option>
          <option value="NATJECATELJSKA">Natjecateljska</option>
          <option value="VRHUNSKA">Vrhunska</option>
        </select>
      </Field>
      ...
      <div className="flex gap-6">
        <label className="flex items-center gap-2 text-sm text-ink">
          ...
          />
          Završni turnir (GP Finale / Prvenstvo Akademije)
        </label>
        <label className="flex items-center gap-2 text-sm text-ink">
          <input
            type="checkbox"
      ...
      {defaultSystem === "AKADEMIJA" && (
        <p className="text-xs text-academy">
          Ova sezona pripada GP Akademiji — razina (FC) i tempo-faktor (FT) se
          ne koriste za bodovanje, samo faktor broja igrača (čl. 8).
        </p>
      )}
