...
import { removeGuardianship } from "./actions";

export function RemoveGuardianshipButton({
  linkId,
  label,
}: {
  linkId: string;
  ...
  return (
    <span className="flex items-center gap-2 text-xs">
      <span className="text-ink/60">Ukloniti {label}?</span>
      <button
        type="button"
        disabled={isPending}
        onClick={() =>
          startTransition(async () => {
            await removeGuardianship(linkId);
          })
        }
